	...
	int downMatrix, int infoOnly, int adts_out, float *song_length)
{
	...
	global.mp4File = fopen(mp4file, "rb");
	global.mp4cb->read = read_callback;
	global.mp4cb->seek = seek_callback;
	global.mp4cb->user_data = global.mp4File;


	global.hDecoder = NeAACDecOpen();

	/* Set configuration */
	config = NeAACDecGetCurrentConfiguration(global.hDecoder);
	config->outputFormat = outputFormat;
	config->downMatrix = downMatrix;
	//config->dontUpSampleImplicitSBR = 1;
	NeAACDecSetConfiguration(global.hDecoder, config);

	if (adts_out)
	{
		global.adtsFile = fopen(adts_fn, "wb");
		if (global.adtsFile == NULL)
		{
			faad_fprintf(stderr, "Error opening file: %s\n", adts_fn);
			return 1;
		}
	...
	global.infile = mp4ff_open_read(global.mp4cb);
	if (!global.infile)
	{
		/* unable to open file */
		faad_fprintf(stderr, "Error opening file: %s\n", mp4file);
	...
	if ((global.track = GetAACTrack(global.infile)) < 0)
	{
		faad_fprintf(stderr, "Unable to find correct AAC sound track in the MP4 file.\n");
	...
	mp4ff_get_decoder_config(global.infile, global.track, &global.buffer, &global.buffer_size);

	if (NeAACDecInit2(global.hDecoder, global.buffer, global.buffer_size,
		&global.samplerate, &channels) < 0)
	{
		/* If some error initializing occured, skip the file */
		faad_fprintf(stderr, "Error initializing decoder library.\n");
		NeAACDecClose(global.hDecoder);
	...
	global.timescale = mp4ff_time_scale(global.infile, global.track);
	global.framesize = 1024;
	global.useAacLength = 0;

	if (global.buffer)
	{
		if (NeAACDecAudioSpecificConfig(global.buffer, global.buffer_size, &mp4ASC) >= 0)
		{
			if (mp4ASC.frameLengthFlag == 1) global.framesize = 960;
			if (mp4ASC.sbr_present_flag == 1) global.framesize *= 2;
		}
		free(global.buffer);
	}
	...
	faad_fprintf(stderr, "%s file info:\n\n", mp4file);
	{
		...
		int k, j;
		char *ot[6] = { "NULL", "MAIN AAC", "LC AAC", "SSR AAC", "LTP AAC", "HE AAC" };
		long samples = mp4ff_num_samples(global.infile, global.track);
		float f = 1024.0;
		float seconds;
		if (mp4ASC.sbr_present_flag == 1)
		{
...
#define PRINT_MP4_METADATA
#ifdef PRINT_MP4_METADATA
		j = mp4ff_meta_get_num_items(global.infile);
		for (k = 0; k < j; k++)
		{
			if (mp4ff_meta_get_by_index(global.infile, k, &item, &tag))
			{
				if (item != NULL && tag != NULL)
				{
					faad_fprintf(stderr, "%s: %s\n", item, tag);
...
static int decodeAACfile_opening(char *aacfile, char *adts_fn, int def_srate, int object_type, int outputFormat, int downMatrix, int infoOnly, int adts_out, float *song_length)
{
	int tagsize;
	unsigned char channels;
	...
	memset(&global.b, 0, sizeof(aac_buffer));
	...
	global.b.infile = fopen(aacfile, "rb");
	if (global.b.infile == NULL)
	{
		/* unable to open file */
		faad_fprintf(stderr, "Error opening file: %s\n", aacfile);
		return 1;
	...
	fseek(global.b.infile, 0, SEEK_END);
	fileread = ftell(global.b.infile);
	fseek(global.b.infile, 0, SEEK_SET);
	...
	bread = fread(global.b.buffer, 1, FAAD_MIN_STREAMSIZE*MAX_CHANNELS, global.b.infile);
	global.b.bytes_into_buffer = bread;
	global.b.bytes_consumed = 0;
	global.b.file_offset = 0;

	if (bread != FAAD_MIN_STREAMSIZE * MAX_CHANNELS)
		global.b.at_eof = 1;

	tagsize = 0;
	if (!memcmp(global.b.buffer, "ID3", 3))
	{
		/* high bit is not used */
		tagsize = (global.b.buffer[6] << 21) | (global.b.buffer[7] << 14) |
			(global.b.buffer[8] << 7) | (global.b.buffer[9] << 0);

		tagsize += 10;
		advance_buffer(&global.b, tagsize);
		fill_buffer(&global.b);
	}
	...
	config->outputFormat = outputFormat;
	config->downMatrix = downMatrix;
	config->useOldADTSFormat = global.old_format;
	//config->dontUpSampleImplicitSBR = 1;
	NeAACDecSetConfiguration(global.hDecoder, config);

	/* get AAC infos for printing */
	header_type = 0;
	if ((global.b.buffer[0] == 0xFF) && ((global.b.buffer[1] & 0xF6) == 0xF0))
	{
		adts_parse(&global.b, &bitrate, &length);
		fseek(global.b.infile, tagsize, SEEK_SET);
		...
		global.b.bytes_into_buffer = bread;
		global.b.bytes_consumed = 0;
		global.b.file_offset = tagsize;

		header_type = 1;
	}
	else if (memcmp(global.b.buffer, "ADIF", 4) == 0) {
		int skip_size = (global.b.buffer[4] & 0x80) ? 9 : 0;
		bitrate = ((unsigned int)(global.b.buffer[4 + skip_size] & 0x0F) << 19) |
			((unsigned int)global.b.buffer[5 + skip_size] << 11) |
			((unsigned int)global.b.buffer[6 + skip_size] << 3) |
			((unsigned int)global.b.buffer[7 + skip_size] & 0xE0);
	...
	*song_length = length;
	...
	fill_buffer(&global.b);
	if ((bread = NeAACDecInit(global.hDecoder, global.b.buffer,
		global.b.bytes_into_buffer, &global.samplerate, &channels)) < 0)
	{
		/* If some error initializing occured, skip the file */
		faad_fprintf(stderr, "Error initializing decoder library.\n");
		if (global.b.buffer)
	...
	case 1:
		faad_fprintf(stderr, "ADTS, %.3f sec, %d kbps, %d Hz\n\n",
			length, bitrate, global.samplerate);
		break;
	case 2:
		faad_fprintf(stderr, "ADIF, %.3f sec, %d kbps, %d Hz\n\n",
			length, bitrate, global.samplerate);
...
int decodeMP4file_iteration()
{
	...
	dur = mp4ff_get_sample_duration(global.infile, global.track, global.sampleId);
	rc = mp4ff_read_sample(global.infile, global.track, global.sampleId, &global.buffer, &global.buffer_size);
	if (rc == 0)
	{
		faad_fprintf(stderr, "Reading from MP4 file failed.\n");
		NeAACDecClose(global.hDecoder);
	...
	global.sample_buffer = NeAACDecDecode(global.hDecoder, &global.frameInfo, global.buffer, global.buffer_size);

	if (global.adts_out == 1)
	{
		global.adtsData = MakeAdtsHeader(&global.adtsDataSize, &global.frameInfo, 0);

		/* write the adts header */
		fwrite(global.adtsData, 1, global.adtsDataSize, global.adtsFile);

		fwrite(global.buffer, 1, global.frameInfo.bytesconsumed, global.adtsFile);
	}

	if (global.buffer) free(global.buffer);

	if (!global.noGapless)
	{
		if (global.sampleId == 0) dur = 0;

		if (global.useAacLength || (global.timescale != global.samplerate)) {
			sample_count = global.frameInfo.samples;
		}
		else {
			sample_count = (unsigned int)(dur * global.frameInfo.channels);
			if (sample_count > global.frameInfo.samples)
				sample_count = global.frameInfo.samples;

			if (!global.useAacLength && !global.initial && (global.sampleId < global.numSamples / 2) && (sample_count != global.frameInfo.samples))
			{
				faad_fprintf(stderr, "MP4 seems to have incorrect frame duration, using values from AAC data.\n");
				global.useAacLength = 1;
				sample_count = global.frameInfo.samples;
		...
		if (global.initial && (sample_count < global.framesize*global.frameInfo.channels) && (global.frameInfo.samples > sample_count))
			delay = global.frameInfo.samples - sample_count;
	}
	else {
		sample_count = global.frameInfo.samples;
	}
	...
	/* open the sound file now that the number of channels are known */
	if (global.first_time && !global.frameInfo.error)
	{
		/* print some channel info */
		print_channel_info(&global.frameInfo);
...
int decodeAacfile_iteration()
{
	...
	if (global.adts_out == 1)
	{
		int skip = (global.old_format) ? 8 : 7;
		global.adtsData = MakeAdtsHeader(&global.adtsDataSize, &global.frameInfo, global.old_format);

		/* write the adts header */
		fwrite(global.adtsData, 1, global.adtsDataSize, global.adtsFile);

		/* write the frame data */
		if (global.frameInfo.header_type == ADTS)
			fwrite(global.b.buffer + skip, 1, global.frameInfo.bytesconsumed - skip, global.adtsFile);
		else
			fwrite(global.b.buffer, 1, global.frameInfo.bytesconsumed, global.adtsFile);
	}

	/* update buffer indices */
	advance_buffer(&global.b, global.frameInfo.bytesconsumed);
