...
BE_ERR	beInitStream(PBE_CONFIG pbeConfig, DWORD *pdwSamples, DWORD *pdwBufferSize,
...
// end Dibrom's CBR preset 2001-12-18
	...
	if ( 0 == lame_get_version( gfp ) )
	{
		...
		*pdwSamples= 576 * lame_get_num_channels( gfp );
	}
	else
	{
		// For MPEG-I, 1152 samples per frame per channel
		*pdwSamples= 1152 * lame_get_num_channels( gfp );
	}

	// Set the input sample buffer size, so we know what we can expect
	dwSampleBufferSize = *pdwSamples;

	// Set MP3 buffer size, conservative estimate
	*pdwBufferSize = (DWORD)( 1.25 * ( *pdwSamples /
		lame_get_num_channels( gfp ) ) + 7200 );
...
BE_ERR	beFlushNoGap(HBE_STREAM hbeStream, BYTE *pOutput, DWORD *pdwOutput)
{
...
BE_ERR	beDeinitStream(HBE_STREAM hbeStream, BYTE *pOutput, DWORD *pdwOutput)
{
	int nOutputSamples = 0;
...
BE_ERR	beEncodeChunk(HBE_STREAM hbeStream, DWORD nSamples,
			 SHORT *pSamples, BYTE *pOutput, DWORD *pdwOutput)
{
	// Encode it
	int dwSamples;
	int	nOutputSamples = 0;
...
BE_ERR	beEncodeChunkFloatS16NI(HBE_STREAM hbeStream, DWORD nSamples,
			FLOAT *buffer_l, FLOAT *buffer_r, BYTE *pOutput, DWORD *pdwOutput)
{
	int nOutputSamples;
	lame_global_flags*	gfp = (lame_global_flags*)hbeStream;
...
BE_ERR beWriteInfoTag( HBE_STREAM hbeStream,
											 LPCSTR lpszFileName )
{
	FILE* fpStream	= NULL;
