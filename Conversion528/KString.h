...
{
public:
	ConversionStrategy() // Explicitly illegal constructor syntax
	{
		assert(false);
	}
};
...
template <>
class ConversionStrategy<2>	// Windows
{
public:
	inline const UTF16*	GetAsUtf16(const KString* pStr);
};

template <>
class ConversionStrategy<4> // Mac and Linux
{
private:
	bool	IncreaseBufferSize(char** ppBuffer, int* pnBufferLength);
public:
	inline const UTF16*	GetAsUtf16(const KString* pStr);
};
...
{
	friend class ConversionStrategy<4>;
	friend class ConversionStrategy<2>;
	typedef ConversionStrategy<sizeof(wchar_t)> DefaultConversionStrategy;
	mutable DefaultConversionStrategy m_ConversionStrategy;
public:
	typedef int index_t;
