...
const char* KString::GetAsUtf8() const
{
	...
	int cbSize = utfslen(m_wszString, nWideLen) + 1;

	delete[] m_szTempString;

	m_szTempString = new char[cbSize];

	wc2utfs(m_szTempString, m_wszString, nWideLen + 1);

	return m_szTempString;
}

const UTF16*	KString::GetAsUtf16() const
{
	return m_ConversionStrategy.GetAsUtf16(this);
}
...
int utf2wcs(wchar_t *dst, const char *src, int n)
{
  ...
  while (p < n)
  {
    ...
    if (0xC0 <= w){ w = (w<<6)^(unsigned char)src[p++]^0x3080;
    if (0x800 <= w){ w = (w<<6)^(unsigned char)src[p++]^0x20080;
    if (0x10000 <= w){ w = (w<<6)^(unsigned char)src[p++]^0x400080;
    if (0x200000 <= w){ w = (w<<6)^(unsigned char)src[p++]^0x8000080;
    if (0x4000000 <= w){ w = (w<<6)^(unsigned char)src[p++]^0x80; }}}}}
...
KString ConvertLatin1ToKString(const char* szString)
{
...
namespace
{
	Asserter<sizeof(UTF32)==4>	utf32isOk;
	Asserter<sizeof(UTF16)==2>	utf16isOk;
	Asserter<sizeof(UTF8)==1>	utf8isOk;
}


bool	ConversionStrategy<4> :: IncreaseBufferSize(char** ppBuffer, int* pnBufferLength)
{
	UTF16* &pBuf = * (UTF16**)ppBuffer;
	int& nLen	= *pnBufferLength;
...
inline const UTF16* ConversionStrategy<4> :: GetAsUtf16(const KString* pStr)
{
	int nBufferLength = 0;
	char* &pBuffer = pStr->m_szTempString;

	size_t length = pStr->GetLength();

	while(nBufferLength <  length + 1) // Initiate buffer length approximation.
	{
		if(!IncreaseBufferSize(&pBuffer, &nBufferLength))
		{
			return 0; // Can't increase
		}
	}

	ConversionResult res;

	UTF16* startOfTarget	= (UTF16*)pBuffer;
	UTF16* endOfTarget		= ((UTF16*)pBuffer) + nBufferLength - 1;

	const UTF32* startOfSource	= (const UTF32*)(const wchar_t*)(*pStr);
	const UTF32* endOfSource	= startOfSource + length;

	res = ConvertUTF32toUTF16(&startOfSource, endOfSource, &startOfTarget, endOfTarget, lenientConversion);

	*startOfTarget = UTF16(0);

	if(res == conversionOK) // All right
	{
		return (const UTF16*) pBuffer;
	}

	if(res == targetExhausted) // insuff. room in buffer for target
	{
		if(!IncreaseBufferSize(&pBuffer, &nBufferLength))
		{
			return 0; // Can't increase
...
inline const UTF16* ConversionStrategy<2> :: GetAsUtf16(const KString* pStr)
{
	assert(false); //Was NOT tested!!!
	return (const UTF16*)(const wchar_t*)(*pStr);
}
...
KString ConvertUtf16ToKString(const UTF16* szString)
{
	if (sizeof(UTF16) == sizeof(wchar_t))
	{
		return KString((const wchar_t*)szString);
	}
