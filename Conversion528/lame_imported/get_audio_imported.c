...
init_outfile(char const *outPath, int decode)
{
	...
	if (0 == strcmp(outPath, "-")) {
		outFileGlobal = stdout;
		lame_set_stream_binary_mode(outFileGlobal);
	}
	else {
		outFileGlobal = lame_fopen(outPath, "w+b");
#ifdef __riscos__
...
init_infile(lame_t gfp, char const *inPath)
{
	...
	return (global.snd_file != NULL || global.music_in != NULL) ? ftell(global.music_in) : -1;
}
...
int put_audio16_imported(short input[2][1152], char output[2 * 1152 * 2], int iread, int nch)
{
	int i, m = 0;
	...
	if (global_decoder.disable_wav_header && global_reader.swapbytes) {
		if (nch == 1) {
			...
			for (i = 0; i < iread; i++) {
				...
				output[m++] = LOW__BYTE(x);
				/* write 16 Bits High Low */
				output[m++] = HIGH_BYTE(y);
				output[m++] = LOW__BYTE(y);
			}
		}
	}
	else {
